const WholePageSlider = class {
  ...
  init() {
    const sectionButtonContainer = this.createElement(
      "div",
    ...
    for (let index = 0; index < this.sections.length; index++) {
      ...
      this.pagesPerSection[index] = this.sections[index].getElementsByClassName(
        this.pageClass
      );

      // Apply background color for section
      if (this.options.colors) {
        this.sections[index].style.background = this.options.colors[index]
      ...
      if (this.pagesPerSection[index].length > 1) {
        ...
        pageButtonContainer.style.left = `calc(50% - ${
          pageButtonContainer.getBoundingClientRect().width / 2
        }px)`;
      }
    }
    // Same thing as pageButtonContainer, but only with height
    sectionButtonContainer.style.top = `calc(50% - ${
      sectionButtonContainer.getBoundingClientRect().height / 2
    }px)`;
  }
  ...
  switchAndTranslateSection(swipeOrClick) {
    ...
    if (
      (swipeOrClick.deltaY > 0 || swipeOrClick === "down") &&
      this.swipeStartDirection !== "up" &&
      this.currentSection < this.sections.length - 1
    ) {
      this.currentSection++;
      this.translate.section -= this.height;
    } else if (
      (swipeOrClick.deltaY < 0 || swipeOrClick === "up") &&
      this.swipeStartDirection !== "down" &&
      this.currentSection > 0
    ) {
      this.currentSection--;
      this.translate.section += this.height;
    ...
    for (let index = 0; index < this.sections.length; index++) {
      this.sections[
        index
      ].style.transform = `translateY(${this.translate.section}%) scale(1)`;
    }
  ...
  switchAndTranslatePage(swipeOrClick) {
    ...
    if (
      swipeOrClick === "right" &&
      this.swipeStartDirection !== "left" &&
      this.currentPage[this.currentSection] <
        this.pagesPerSection[this.currentSection].length - 1
    ) {
      this.currentPage[this.currentSection]++;
      this.translate.page[this.currentSection] -= this.width;
    } else if (
      swipeOrClick === "left" &&
      this.swipeStartDirection !== "right" &&
      this.currentPage[this.currentSection] > 0
    ) {
      this.currentPage[this.currentSection]--;
      this.translate.page[this.currentSection] += this.width;
    } else if (swipeOrClick.type === "click") {
      const getDirectionFromClick =
        parseInt(swipeOrClick.target.value) -
        this.currentPage[this.currentSection];
      this.currentPage[this.currentSection] = parseInt(
        swipeOrClick.target.value
      );
      this.translate.page[this.currentSection] =
        this.translate.page[this.currentSection] -
        this.width * getDirectionFromClick;
    } else {
      // Now, if there was any dragging, but canceled – animate back to origin.
      this.translate.page[this.currentSection] =
        Math.round(this.translate.page[this.currentSection] / 100) * 100;
    }
    ...
    const button = document.getElementById(
      `page[${this.currentSection}][${this.currentPage[this.currentSection]}]`
    );
    if (button) {
      button.checked = true;
    }
  ...
  draggingEffect() {
    ...
    if (
      ...
      // } else if (this.swipeStartDirection === "left") {
      //   this.width -= this.draggingPercent;
      //   this.translate.page[this.currentSection] += this.draggingPercent;
      // }
      ...
      //   pages[index].style.transform = `translateX(${
      //     this.translate.page[this.currentSection]
      //   }%)`;
      // }
    }
    ...
    if (
      ...
      for (let index = 0; index < this.sections.length; index++) {
        this.sections[
          index
        ].style.transform = `translateY(${this.translate.section}%) scale(0.8)`;
      }
    }
  ...
  touchMove(event) {
    ...
    if (
      Math.abs(this.touches.differenceX) > Math.abs(this.touches.differenceY)
    ) {
  ...
  swipeWithKeyboard(event) {
    ...
    } else if (event.keyCode === 40 || event.code === "ArrowDown") {
  ...
  createElement(tag, options, parent) {
    try {
      const getParent =
        typeof parent === "object" ? parent : document.getElementById(parent);
